...
| `offset` | vec2 | Optional. Nudge **after** the named cell, local XY. Absent = (0, 0). |
...
**When present:** `point` (and optional `height`) names which cell of the local axis-aligned bounds coincides with local (0,0,0) (and with [`rig.spatial.transform`](transform.md) `position` when that is present). Then add `offset`. The host offsets from authored bounds to honour it — geometry field layouts do not change. Interior directions follow the host axes: an origin at top-left on a Y-up bed means the interior is +X and −Y.
